...
interface Props {}
const SelectedDashOneobetCasino: React.FC<Props> = observer(({ ...props }) => {
  ...
  const handleOnChangeGame = async(item) => {
    ...
    const data = tempData?.find((z) => z === item?._id);
    if (data) {
      // eslint-disable-next-line eqeqeq
      const filterData = tempData?.filter((z) => z != item?._id);
      tempData = filterData;
      ...
      await loadSaveAsianDashboard(payload);
    } else {
      setIsSelectedGames([...tempData, item?._id]);
      tempData = [...tempData, item?._id];
      const payload = {
        tableId: item?.tableId,
        isDashboard: true
      };
  ...
  const handleSearchGames = async (pageSize = 0) => {
    ...
    if (res?.success) {
      ...
      const filterGame = res?.allAsianTables
        ?.filter((z) => z?.isDashboard === true)
        ?.map((item) => item?._id);
      setIsSelectedGames(filterGame);
      setScrolledImages(res?.allAsianTables);
      setIsFirst(false)
    }
  } catch(error){
    setIsFirst(false)
    ...
    <div className={style.mainWrapper}>
      {isFirst ? (
        ...
        <>
          ...
          >
            <TitleBarUpdated className={style.mainHeading} title={`1obet Dashboard Game`}/>
          </Row>
                        ...
                        >
                          ...
                          >
                            <img
                              ...
                              src={`${oneobetCasinoUrl}${item?.imageUrl}`}
                              alt="logo"
                            />
                            <div className={style.textOverlay}>
                              {isSelectedGames?.find((z) => z === item?._id) ? (
                                <CheckCircleFilled />
                              ) : (
                                ""
                              )}
                              <p
                                title={
                                  item?.tableName?.length > 16 ? item?.tableName : ""
                                }
                              >
                                {truncate(item?.tableName, 16)}
                              </p>
                            </div>
                          </div>
                        </Col>
...
export default memo(SelectedDashOneobetCasino);
