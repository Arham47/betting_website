...
export const SportsHighlight = observer(() => {
  const {
    user: {
      getUserInfo,
  ...
  const handleGetCurrentPositionData = async () => {
    ...
    const response = await loadGetHighlight2(payload);
    ...
    if (response?.success) {
      ...
      if (cricket?.length) {
        ...
        //get unique object on the base of market id
        ...
        // console.log(`Accccccc`, uniqueMarketObjects);
      ...
      console.log("-----------------", dummyCurrentPositionObj);
  ...
  const getSportsAmounts = (item, data) => {
    if (item === "Soccer") {
      ...
      return (
        <p style={{ color: "green" }}>
          {numberWithCommas(Math.round(data?.totalMatched))}
        </p>
      );
    }
  };
