...
import {BsArrowRepeat, BsCardChecklist, BsFillVolumeMuteFill, BsFillVolumeUpFill} from "react-icons/bs";
// import style from "../style.module.scss";
...
const AllMatchesTable: React.FC<AllMatchesTableProps> = observer(
  ({
    ...
    console.log('000000000000',liveScoreData);
  ...
  // const [speakable, setSpeakable] = useState('true');
  const [prevScoreStatus, setPrevScoreStatus] = useState(null);
    ...
    console.log('555555555' ,liveScoreData);
    const setSpeaker = (val) => {
      localStorage.setItem('speak', val)
      setSpeakable(val)
    }

         //Text to voice ScoreStatus
  useEffect(() => {
    const currentScoreStatus = liveScoreData?.score?.spnballrunningstatus;
    if (currentScoreStatus && currentScoreStatus !== prevScoreStatus) {
      // console.log('speakable -----> ', speakable, localStorage.getItem('speak'))
      if (speakable === 'true' && localStorage.getItem('speak') === 'true') {
        // console.log('speakable speaking', currentScoreStatus)
        // speak({text: currentScoreStatus});

      }
      setSpoken(true);

    } else if (!currentScoreStatus && spoken) {
      setSpoken(false);

    }

    // Update prevScoreStatus
    setPrevScoreStatus(currentScoreStatus);
                ...
                >
                 ...
                 <div style={{display:'flex', justifyContent:'space-between'}}>
                      <span style={{display:'flex', gap:8}}>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.spnnation2 || ''}</b>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.score2 || ''}</b>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.activenation1 === 1 ? liveScoreData?.score?.spnreqrate1 : liveScoreData?.score?.spnreqrate2}</b>
                      </span>
                      <span style={{display:'flex', gap:8}}>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.spnnation1 || ''}</b>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.score1 || ''}</b>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.activenation1 === 1 ? liveScoreData?.score?.spnrunrate1 : liveScoreData?.score?.spnrunrate2}</b>
                      </span>


                      <span style={{display:'flex', gap:8}}>
                        <b style={{ color: 'orange' }}>{liveScoreData?.score?.spnballrunningstatus || ''}</b>
                        <b style={{ color: 'white' }}>{liveScoreData?.score?.comment || ''}</b>
                      </span>
...
speaker
                      ...
                      <span style={{display:'flex', gap:8}}>
                      <b style={{ color: 'white' }}>{targetHandler(liveScoreData)}</b>
                      </span>
                  </div>
                </div>
              ) : (
      ...
      <MainTableWrraper>
        <Row className={style.eventHeader}>
            ...
            title={<span className={style.mainSpan}> { `${innerWidth > 700 ? liveMatchData?.name : truncate(liveMatchData?.name, 36)|| ''} - Match Odds`} <span>{raceStatus===null ?liveMatchData?.status:  raceStatus?.inplay?" Inplay " : "CLOSED"}</span><span className={style.userIcon}><FaIdBadge onClick={() => {
              ...
              {localStorage.getItem('speak') === 'true'
                          ? <BsFillVolumeUpFill style={{ color: 'white', fontSize: '24px', fill: 'white' }}  onClick={() => setSpeaker('false')}/>
                          : <BsFillVolumeMuteFill style={{ color: 'white', fontSize: '24px', fill: 'white' }}  onClick={() => setSpeaker('true')}/>
                }
