...
const AdminCricketOdds = observer(() => {
  ...
  const {
    ...
    const handleBookerProfitLoss = (data, betsData) => {
      ...
      if(betsData?.length){
        temp?.forEach((z, idx) => {
          ...
          temp[idx]['profitLoss'] = Number(dummy?.reduce((accumulator, item) => {return accumulator + item.profitVal}, 0));
          // })
        })
      }
    ...
    if(betsData?.length){
      temp?.forEach((z, idx) => {
        ...
        temp[idx]['profitLoss']  = Number(dummy?.reduce((accumulator, item) => {return accumulator + item.profitVal}, 0));
        temp[idx]['profitLoss2'] = Number(dummy2?.reduce((accumulator, item) => {return accumulator + item.profitVal}, 0));
        // })
      })
    }
  ...
  const bettingFigureHeading = (data) => {
    ...
    if(data?.score?.activenation1 == 1){
      const splitedScore  = data?.score?.score1?.split("&");
      const singleScore   = splitedScore[splitedScore.length -1];
      currentOver = Number(singleScore?.split('(')[1]?.replaceAll(')', ''));
      playingTeam = data?.score?.spnnation1;
    } else if(data?.score?.activenation2 == 1){
      const splitedScore  = data?.score?.score2?.split("&");
      const singleScore   = splitedScore[splitedScore.length -1];
      currentOver = Number(singleScore?.split('(')[1]?.replaceAll(')', ''));
      playingTeam = data?.score?.spnnation2;
    }

    // console.log("================ score ", currentOver);
  ...
  const bettingFigureHeadingSession = (data) => {
    ...
    if(allMarketIdData?.matchType == "TEST"){
      ...
      // khawar
      if(day > 1) 
        currentSessionWith2ndTeam = currentSessionWith2ndTeam + 18
  ...
  return (
    <div className={style.mainCricketWrraper}>
        {isMobile ? <Draggable style={{ position: 'fixed', left:'none', right: '10px', top: '52x', zIndex: 99999 , cursor: 'move'}}>
        ...
        <Row className={style.mainRow} gutter={6}>
          <Col md={24} lg={16} xxl={16}>
            <div className={style.mainHeader}>
              {width < 991
                ? !(getUserInfo?.role === NUM_STR_5_ROLE) && (
                    ...
                    <CustomButton customClass={style.btnStyle} title = {"User Book"} onClick={()=> {
                      navigate(`${constRoute?.userBookCricket}?id=${allMarketIdData?._id}`);
                      const otherEvents = JSON.parse(localStorage.getItem('userBookEvent'));
                      localStorage.setItem('userBookEvent', JSON.stringify({...otherEvents, cricket:allMarketIdData?.name}))
                      }}/>
                  </div>}
                    />
                  )
              ...
              <Row className={style.scoreSection}>
                ...
                <div className={style.mainScoresWrraper}>
                  ...
                  {bookMakerFancyData?.length ?  <div className={style.mainScoresWrraper}>
                  <AllMatchesTable
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
                    copyDataOfFakeBets={lastMarketIdData}
                ...
                {marketIdsData?.filter((item)=>item?.marketName?.includes('Tied Match'))?.length? <div className={style.mainScoresWrraper}>
                  <AllMatchesTable
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
                    copyDataOfFakeBets={lastMarketIdData}
                    // tableData={marketIdsData}
                    tableData={getMinutesCalculate(allMarketIdData?.openDate) ?  marketIdsData?.filter((item)=>!item?.marketName?.toLowerCase()?.includes('to win the toss')&&item?.marketName?.includes('Tied Match')): marketIdsData?.filter((item)=>item?.marketName?.includes('Tied Match'))}
                    ...
                    raceStatus={raceStatus}
                    isFancyBooker={true}
                    marketIdsForProfitLoss={getMinutesCalculate(allMarketIdData?.openDate) ?  marketIdsForProfitLoss?.filter((item)=>!item?.marketName?.toLowerCase()?.includes('to win the toss')&&item?.marketName?.includes('Tied Match')): marketIdsForProfitLoss?.filter((item)=>item?.marketName?.includes('Tied Match'))}
                    getMatchBetsData={getMatchedBetsData}
                    currentPositinProfitLoss={currentPositinProfitLoss}
                  />
                </div>:''}
                {fancyDataApi?.length? <div className={style.mainScoresWrraper}>
                  <AllMatchesTable
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
                    copyDataOfFakeBets={lastMarketIdData}
                  ...
                  {/* <div className={style.mainScoresWrapper}>
                  <AllMatchesTable
                    // setBetPlayType={setBetPlayType}
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
                    copyDataOfFakeBets={lastMarketIdData}
                    tableData={getTimeDiff(allMarketIdData?.openDate)?.includes('ago') ?  marketIdsData?.filter((item)=>item?.marketName!='To Win The Toss'&&item?.marketName!='Tied Match'): marketIdsData?.filter((item)=>item?.marketName!='Tied Match')}
                ...
                {bookMakerFancyData?.length ? <div className={style.mainScoresWrapper}>
                    <AllMatchesTable
                    // setBetPlayType={setBetPlayType}
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
                  ...
                  {marketIdsData?.filter((item)=>item?.marketName?.includes('Tied Match'))?.length ? <div className={style.mainScoresWrapper}>
                    <AllMatchesTable
                    // setBetPlayType={setBetPlayType}
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
                  ...
                  {fancyDataApi?.length ? <div className={style.mainScoresWrapper}>
                    <AllMatchesTable
                    // setBetPlayType={setBetPlayType}
                    isEditStake={isEditStake}
                    setIsEditStake={setIsEditStake}
            ...
            <div className={style.figureBetting}>
              <TitleBarUpdated 
                ...
                {bettingFigureHeading(liveScoreData)}<span className={style.userIcon}><FaIdBadge onClick={() => {
                  navigate(`${constRoute?.userBookCricket}?id=${allMarketIdData?._id}`);
                  const otherEvents = JSON.parse(localStorage.getItem('userBookEvent'));
                  localStorage.setItem('userBookEvent', JSON.stringify({...otherEvents, cricket:allMarketIdData?.name}))
                  }}/></span>
              </span>}
              isRightRibbon={<span  className={style.zeroCommission}>(0% commission)</span>}
            />
            </div>
                ...
                {getLoadAllBettingFiguresList?.map((item, index) => {
                  return (
                    <div className={style.mainWrapper}>
                      ...
                      <div className={style.bettingVal}>{item?.amount}</div>
                      <div className={style.bettingValBottom} style={{color: bettingFigureProfitLoss && bettingFigureProfitLoss[index]?.profitLoss>0 ? 'green' : 'red'}}>{Number(bettingFigureProfitLoss && Math.round(Number(bettingFigureProfitLoss[index]?.profitLoss))) || ''}</div>
                    </div>
                  );
                })}
              </div>
                  ...
                  <h1>JOTTA</h1>
                    <div className={style.jottaNewWrapper}>
                      ...
                      <KaliJotaHead bg={"#8DD2F0"}>
                        <span>100<span style={{fontSize: '12px', color: 'brown'}}>%</span></span><span style={{fontSize:'14px', color:Number(jottaKaliProfitLoss?.JotaProfitLoss)>0 ? 'green' : 'red', fontWeight:'bold'}}>{getMatchedBetsData?.some((item) => (item?.runnerId == "JOTTA" && item?.betSession==bettingFigureHeadingSession(liveScoreData)) || (item?.runnerId == "KALI" && item?.betSession==bettingFigureHeadingSession(liveScoreData))) ? Math.round(Number(jottaKaliProfitLoss?.JotaProfitLoss)) : ''}</span>
                      </KaliJotaHead>
                    </div>
                  </div>
                  <div className={style.jottaMain}>
                    <h1>KALI <span>(0% commission)</span></h1>
                    <div className={style.jottaNewWrapper}>
                      <KaliJotaHead bg={"#FFB2AF"}>
                        <b>𝗞</b>
                      </KaliJotaHead>
                      <KaliJotaHead bg={"#8DD2F0"}>
                      <span>100<span style={{fontSize: '12px', color: 'brown'}}>%</span></span><span style={{fontSize:'14px', color:Number(jottaKaliProfitLoss?.kaliProfitLoss)>0 ? 'green' : 'red', fontWeight:'bold'}}>{getMatchedBetsData?.some((item) => (item?.runnerId == "KALI" && item?.betSession==bettingFigureHeadingSession(liveScoreData)) || (item?.runnerId == "JOTTA" && item?.betSession==bettingFigureHeadingSession(liveScoreData))) ? Math.round(Number(jottaKaliProfitLoss?.kaliProfitLoss)) : ''}</span>
                      </KaliJotaHead>
                    </div>
                  </div>
                </div>
                ...
                <div className={style.bothWrapperNew}>
                  <div className={style.jottaMain}>
                    ...
                    <div className={style.chottaBarraWrapper}>
                      ...
                      <KaliJotaHead bg={"#8DD2F0"}>
                      <span>100<span style={{fontSize: '12px', color: 'brown'}}>%</span></span><span style={{fontSize:'14px', color:Number(chottaBrraProfitLoss?.chottaProfitLoss)>0 ? 'green' : 'red', fontWeight:'bold'}}>{getMatchedBetsData?.some((item) => (item?.runnerId == "CHOTA" && item?.betSession==bettingFigureHeadingSession(liveScoreData)) || (item?.runnerId == "BARA" && item?.betSession==bettingFigureHeadingSession(liveScoreData))) ? Math.round(Number(chottaBrraProfitLoss?.chottaProfitLoss)) : ''}</span>
                      </KaliJotaHead>
                    </div>
                  </div>
                  <div className={style.jottaMain}>
                    <h1>BARA <span>(0% commission)</span></h1>
                    <div className={style.chottaBarraWrapper}>
                      <KaliJotaHead bg={"#FFB2AF"}>
                        <b>𝗕𝗥</b>
                      </KaliJotaHead>
                      <KaliJotaHead bg={"#8DD2F0"}>
                      <span>100<span style={{fontSize: '12px', color: 'brown'}}>%</span></span><span style={{fontSize:'14px', color:Number(chottaBrraProfitLoss?.barraProfitLoss)>0 ? 'green' : 'red', fontWeight:'bold'}}>{getMatchedBetsData?.some((item) => (item?.runnerId == "BARA" && item?.betSession==bettingFigureHeadingSession(liveScoreData)) || (item?.runnerId == "CHOTA" && item?.betSession==bettingFigureHeadingSession(liveScoreData))) ? Math.round(Number(chottaBrraProfitLoss?.barraProfitLoss)) : ''}</span>
                      </KaliJotaHead>
                    </div>
                  </div>
                </div>
          ...
          >
            ...
            <div>
              ...
              {getUserInfo?.role == "0" ? <CricketCurrentPosition tableData={secondInUseData} updateBetPlaceHoldHandler={loadingUpdateBetPlaceHold ? "" : updateBetPlaceHold} loadingUpdateBetPlaceHold={loadingUpdateBetPlaceHold} secondsValue={secondsValue}/> : ''}
            </div>
...
export default AdminCricketOdds;
