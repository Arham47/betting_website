...
const OpenBetTable = observer(({tableData, ...props}) => {
  ...
  const columns = useMemo(
    () => [
      {
        title: `Runner`,
        render: (_, data) => <div className="dynamicColorOnHover" style={{color:getUserBetsLength(data?.bettor, data?.marketId) ? "red" : ''}}>{data?.runnerId || ''}</div>,
      },
      {
        title: `Price`,
        render: (_, data) => <div className="dynamicColorOnHover" style={{color:getUserBetsLength(data?.bettor, data?.marketId) ? "red" : ''}}>{data?.price || 0}<small style={{ whiteSpace: "nowrap"}}> ({ data?.createdAt ? moment(data?.createdAt).format('hh:mm A') : "--" })</small></div>,
      },
      {
        title: `Size` ,
