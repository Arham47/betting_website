...
const SoccerMarketWise = observer(() => {
  ...
  const handleNewOddsData = (data) => {
    ...
    const filteredMarketIds = data?.marketIds.filter((item) => item?.id == queryMarketId)
    // setAllMarketIdData(filteredMarketIds)
    console.log('============Test======', data)
    setMarketIdsData(filteredMarketIds);
    console.log('============Test1======', filteredMarketIds)
          ...
          >
            {width > 991
              ? !(getUserInfo?.role === NUM_STR_5_ROLE) && (
                  ...
                  <CustomButton customClass={style.btnStyle} title = {"User Book"} onClick={()=> {
                    navigate(`${constRoute?.userBookSoccer}?id=${allMarketIdData?._id}&marketId=${queryMarketId}`);
                    // id=${item?.mId}&marketId=${item?.marketId}&market=${item?.subMarketId}
                    const otherEvents = JSON.parse(localStorage.getItem('userBookEvent'));
                    localStorage.setItem('userBookEvent', JSON.stringify({...otherEvents, soccer:allMarketIdData?.name}))
                    }}/>
                </div>}
...
export default SoccerMarketWise;
