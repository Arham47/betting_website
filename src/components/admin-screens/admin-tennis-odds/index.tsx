...
const AdminTennisOdds = observer(() => {
  ...
  // console.warn('liveScoreData', liveScoreData)
  const {
    user: {
      getAllUsers,
      loadAllUsers,
                    ...
                    >
                      <a onClick={(e) => e.preventDefault()}>
                        <CustomButton
                          ...
                          icon={<DownOutlined className={style.downIcon}/>}
                        />
                      </a>
                    </Dropdown>
                    <CustomButton customClass={style.btnStyle} title = {"User Book"} onClick={()=> {
                      navigate(`${constRoute?.userBook}?id=${allMarketIdData?._id}`);
                      const otherEvents = JSON.parse(localStorage.getItem('userBookEvent'));
                      localStorage.setItem('userBookEvent', JSON.stringify({...otherEvents, tennis:allMarketIdData?.name}))
                      }}/>
                  </div>}
                    />
                  )
...
export default AdminTennisOdds;
