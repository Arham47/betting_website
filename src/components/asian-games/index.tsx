...
interface Props {}
const AsianGames: React.FC<Props> = observer(({ ...props }) => {
  const [selectedCategory, setSelectedCategory] = useState("");
  const [allSelectedCasinosList, setAllSelectedCasinosList] = useState(null);
  ...
  const handleGameListIframe = async (data) => {
    ...
    window.open(`${window.location.href.replaceAll("/all-world-casino",`${constRoute.relatedGame}?hasId=${data?.id_hash}`)}`,"_blank");
  ...
  const handleLoadSelectedCasinosGames = async () => {
    const payload = {
      numRecords,
      isMobile,
      gameCategory: selectedCategoryName ==='All'? '': selectedCategoryName,
    };
    ...
    if (isFirst) setSelectedCategory(res?.categories[0]?.category);
    setIsFirst(false);
  };
  useEffect(() => {
...
export default memo(AsianGames);
