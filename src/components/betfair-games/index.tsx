...
interface Props {}
const BetfairGames: React.FC<Props> = observer(({ ...props }) => {
  const location = useLocation()
  const { user: { getUserInfo },
    bet: {loadingAllSelectedCasinos, loadAllSelectedCasinos },
  } = useStore(null);
  ...
  const handleLoadSelectedCasinosGames = async () => {
    const payload = {
      numRecords,
      isMobile,
      gameCategory: gameCategory?.replaceAll('-', ' '),
    };
...
useEffect(() => {
 if(gameCategory) handleLoadSelectedCasinosGames();
  ...
  const handleGameListIframe=async(data)=>{
    ...
    const newWindow = window.open(`${window.location.href.replaceAll("/betfair-games?name=BF-Games",`${constRoute.relatedGame}?hasId=${data?.id_hash}`)}`,"_blank");
  ...
  return (
    <div className={style.mainWrapper}>
       <Row style={{justifyContent:'center'}}>
        <Col span={22}><TitleBarUpdated title={`Betfair Casino`} /></Col>
      </Row>
...
export default memo(BetfairGames);
