...
const BookDetailDetailPage = observer(() => {
  const {
    user: {loadMarketMatchWiseDetailedData, loadingBookDetailMatchWiseReportData, loadDailyPlMatchWiseDetailedData, loadingPlDailyMatchWiseReportData}, 
  } = useStore(null);
  ...
  const parsData = JSON.parse(localStorage.getItem('bookDetailMarketShare'))
...
const res =   await loadMarketMatchWiseDetailedData(data)
...
setUserRecord(res)
console.log('loadDailyPlMatchWiseDetailedDataresssssssss', res)
}
useEffect(()=>{
  ...
  return (
    ...
    {!isDetailed? <div className={style.mainContainer}>
      <div className={style.container}>
          ...
          <TitleBarUpdated isButton={true} btnTitle={'Back'} clickHandler={() => console.log('ok')} title={<span className={style.userNameDesign} >{parsData?.userName}</span>} isRightRibbon={<div className={style.eventHead}>{parsData?.Data?.name}</div>}/>
        <Row className={style.mainTableDiv}>
          <Col lg={7} md={10} sm={24} xs={24} className={style.gridOne}>
            <Table
              className={style.positiveClient}
              columns={columns}
              loading={loadingBookDetailMatchWiseReportData}
                ...
                <>
                  ...
                  <tr key={index}>
                    ...
                    {data > 450 && data <= 850 ? (
                      ...
                      <>
                        <td className={style.width9}>{item?.size || '--'}</td>
                        {/* letters?.find((z)=>z?.type==item?.type)?.letter */}
...
export default memo(BookDetailDetailPage);
