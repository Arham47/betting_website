...
const DailyPlNewReportUpdated = observer(() => {
  const {
    user: { loadDailyPlMatchWiseDetailedData, loadingPlDailyMatchWiseReportData}, 
  } = useStore(null);
  ...
  const parsData = JSON.parse(localStorage.getItem('marketShareDataPl'))
...
const res =   await loadDailyPlMatchWiseDetailedData(data)
...
localStorage.setItem('currentLink', '/users')
  ...
  return (
        ...
        <Row className={style.mainTableDiv}>
          <Col lg={7} md={10} sm={24} xs={24} className={style.gridOne}>
            <Table
              ...
              columns={columns}
              loading={loadingPlDailyMatchWiseReportData}
    ...
    (betterDetailed?.length && betterDetailed[0]?.sportsId === "6") ? 
    <div>
...
export default memo(DailyPlNewReportUpdated);
