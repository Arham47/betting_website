...
const DailyPl = observer(() => {
  ...
  const openNewWindow = (route) => {
    // const newWindow = window.open(window.location.href.replaceAll('/daily-pl', '/daily-report/MarketShares'), '_blank');
    // localStorage.setItem('currentLink', route)
    // newWindow.focus();
    const newWindow =  window.open(`${window.location.href.replaceAll("/daily-pl",'/daily-pl/MarketShares')}`,"_blank", 'width=520,height=600,resizable,scrollbars');
  ...
  const columnsNegative = [
    ...
    },
    {
      title: CAP_AMOUNT,
      render: (_, data) => (
        <div style={{ color: "red" }}>{numberWithCommas(Math.round(data?.amount))}</div>
      ),
    },
  ];
  const sportColumns = [
    ...
    },
    {
      ...
      render: (_, data) => (
        <div style={{ color: data?.amount > 0 ? "" : "red" }}>
          {numberWithCommas(Math.round(data?.amount))}
        </div>
      ),
    },
  ];
      ...
      render: (_, data) => (
        <div
          onClick={() => {
            ...
            localStorage.setItem('marketShareDataPl', JSON.stringify(newData))
    ...
    },
    {
      ...
      render: (_, data) => (
        <p style={{ color: data?.winningAmount > 0 ? "" : "red" }}>
          {numberWithCommas(Math.round(data?.amount)||0)}
        </p>
      ),
    },
  ];
                ...
                footer={() => {
                  return (
                    <div className={style.footerTotal} style={{background:sportWiseData?.map((item) => item?.amount)?.reduce((a, b) => {return a + b;}, 0)>0?'#085845' : '#F86C6B'}}>
                      <h3 style={{color:'#fffff'}}>{CAP_TOTAL}</h3>
                      <h3 style={{color:'#fffff'}}>
                        {numberWithCommas(
                         Math.round(sportWiseData?.map((item) => item?.amount)?.reduce((a, b) => {return a + b;}, 0) || 0)
                        )}{" "}
                      </h3>
                    </div>
                  );
