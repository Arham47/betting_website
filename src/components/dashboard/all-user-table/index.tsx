...
const AllUsersTable: React.FC<UserTable> = observer(
  ({
    ...
    const fetDataonPageNumber = async () => {
      ...
      }&${LOWER_PAGE_PARAM_KEY}=${pageNumber}&${LOWER_NUM_RECORDS_PARAM_KEY}=${numRecords}`;
      if (isUserId) {
    ...
    return (
      <div className={style.allUsersTable}>
                ...
                {data >= 450 && data < 885 ? (
                  ...
                  <>
                    ...
                    <th className={style.width10}>
                      <div className={style.colorClass} style={{color: loadBalanceTotal?.availableBalance>=0 ? '' : 'red'}}>
                        {" "}
              ...
              <tr>
                ...
                <ul className={style.paddingLeft22}>
                  <li className={style.listStyleFont14}>
                    ...
                    <span className={style.paddingLeft10} style={{color: loadBalanceTotal?.availableBalance>=0 ? '' : 'red'}}>
                      {(loadBalanceTotal?.availableBalance < 0.1 && loadBalanceTotal?.availableBalance > -1) ? 0 : formatLargeNumber(loadBalanceTotal?.availableBalance, 1)?.toString()?.replace('.0', '') || 0}
                    </span>
                  </li>
                </ul>
                ...
                return (
                  <>
                    ...
                    <tr key={index}>
                      ...
                      {data >= 450 && data < 885 ? (
                        <td className={style.width9} style={{color: item?.credit>=0? '': 'red'}}>{item?.role != 0 ? formatLargeNumber(item?.credit, 1)?.toString()?.replace('.0', '') : '--'}</td>
                      ) : data >= 885 && data < 1095 ? (
                        <>
                          <td className={style.width9} style={{color: item?.credit>=0? '': 'red'}}>{item?.role != 0 ? formatLargeNumber(item?.credit, 1)?.toString()?.replace('.0', '') : '--'}</td>
                          <td className={style.width9} style={{color: item?.balance>=0? '': 'red'}}>{item?.role != 0 ?  formatLargeNumber( item?.balance, 1)?.toString()?.replace('.0', '') :'--'}</td>
                          <td className={style.width9} style={{color: item?.clientPL>=0? '': 'red'}}>{item?.role != 0 ? formatLargeNumber(Number(item?.clientPL?.toString()?.replaceAll('e-', '')), 1)?.toString()?.replace('.0', '') :'--'}</td>
                        ...
                        <>
                          <td className={style.width9} style={{color: item?.credit>=0? '': 'red'}}>{item?.role != 0 ? formatLargeNumber(item?.credit, 1)?.toString()?.replace('.0', '') : ''}</td>
                          <td className={style.width10} style={{color: item?.balance>=0? '': 'red'}}>{item?.role != 0 ? formatLargeNumber(Number(item?.balance?.toString()?.replaceAll('e-', '')), 1)?.toString()?.replace('.0', '') :'--'}</td>
                          <td className={style.width10} style={{color: item?.clientPL>=0? '': 'red'}}>{item?.role != 0 ? formatLargeNumber(Number(item?.clientPL?.toString()?.replaceAll('e-', '')), 1)?.toString()?.replace('.0', '') :'--'}</td>
                      ...
                      <tr>
                        ...
                        <ul className={style.paddingLeft22}>
                          ...
                          <li className={style.listStyleFont14}>
                            ...
                            <span className={style.paddingLeft10} style={{color: item?.availableBalance>=0? '': 'red'}}>
                              {(item?.availableBalance < 0.1 && item?.availableBalance > -1) ? 0 : formatLargeNumber(Number(item?.availableBalance?.toString()?.replaceAll('e-', '')), 1)?.toString()?.replace('.0', '') || 0}
                            </span>
                          </li>
                          <li className={style.listStyleFont14}>
        ...
        <ConfirmationModal modelTitle="Delete User" isOpen={isOpenConfirm} onCancel={handleConfirmCancel} isConfirmDisable={loadingDeleteUser} onConfirm={confirmHandler}/>
        {/* </>} */}
      </div>
    );
  }
