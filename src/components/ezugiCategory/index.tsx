...
interface Props {}
const EzugiCategory: React.FC<Props> = observer(({ ...props }) => {
  const location = useLocation()
  const { user: { getUserInfo },
    bet: {loadAllSelectedCasinos, loadingAllSelectedCasinos, loadGetGameListIframeLink },
  } = useStore(null);
...
useEffect(()=>{
  ...
  const handleGameListIframe=async(data)=>{
    ...
    const newWindow = window.open(`${window.location.href.replaceAll("/ezugi-casino?name=ezugi",`${constRoute.relatedGame}?hasId=${data?.id_hash}`)}`,"_blank");
    setIsGameId(null)
  }
  ...
  return (
    <div className={style.mainWrapper}>
       <Row style={{justifyContent:'center'}}>
        <Col span={22}><TitleBarUpdated title={`Ezugi Casino`} className={style.mainHeading}/></Col>
      </Row>
...
export default memo(EzugiCategory);
