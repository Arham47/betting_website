...
export const LedgerCash = observer(() => {
  ...
  const getCashDepositLedgerData = () => {
    ...
    if (firstDate.getTime() && lastDate.getTime()) {
      if (startDate <= endDate) {
        ...
        dispatch(fetchUserForDealerWithDepositedCash(payload)); // Dispatch the Redux action
      } else {
  ...
  const handleExcelFileDownload = () => {
    const data_array = depositedCash.docs.map((item, index) => ({
      ..._.pick(
        {
          "#": index + 1,
  ...
  const openNewWindow = (isAdmin, route) => {
    if (isAdmin) {
      const newWindow = window.open(
        `${window.location.href.replaceAll(`/ledgerCash`, "/statement")}`,
        "_blank",
      ...
      <td colSpan={4} style={{ textAlign: "center" }}>
        <div style={{ marginTop: "20px" }}>
          <Spin tip="Loading..." />
            ...
            ) : depositedCash.docs && depositedCash.docs.length > 0 ? (
              depositedCash.docs.map((item, index) => (
                <tr key={index}>
