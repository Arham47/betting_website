...
export const LedgerReportTwo = observer(() => {
  ...
  const {
    user: {
      ...
      getAllLedgerListData2,
      // getLoadingLedgerDetal,
      getLoadingLedgerDetal2,
      getSingleUser,
      loadSingleUser,
      // getLedgerDetailTotalRecords,
      getLedgerDetailTotalRecords2,
      getUserInfo,
      hanldeSortedData,
      hanldeSortData
    },
  ...
  const getCashDepositLedgerData = async () => {
    ...
    await loadGetDetailLedgerDetail2(payload);
  };
  useEffect(()=>{
    window.scrollTo(0, 0);
  }, []);
  ...
  const handleExcelFileDownload = () => {
    const data_array = getAllLedgerListData2.map((item, index) => ({
      ..._.pick(
        {
          "#": index + 1,
  ...
  const openNewWindow = (isAdmin, route) => {
    if(isAdmin){
      const newWindow =  window.open(`${window.location.href.replaceAll(`/ledger-two/${id}`,'/statement')}`,"_blank", 'width=520,height=600,resizable,scrollbars');
      localStorage.setItem('currentLink', route)
      newWindow.focus();
    }else{
      const newWindow =  window.open(`${window.location.href.replaceAll(`/ledger-two/${id}`,'/statement-detail')}`,"_blank", 'width=520,height=600,resizable,scrollbars');
      localStorage.setItem('currentLink', route)
      newWindow.focus();
    }
  };
  ...
  useEffect(() => {
    const ids = getAllLedgerListData2?.map((item) => item?._id)
    setIsShowData(ids)
  }, [getAllLedgerListData2, data])
  ...
  const handleSorting =()=>{
    const temp = JSON.parse(JSON.stringify(getAllLedgerListData2))
    let sortedData = []
    ...
    <Wrapper>
      ...
      <FlexWrapper className={style.mainWrapper}>
        <div className={style.flexAndGapClass}>
            ...
            />
          <CustomButton
          disabled={getLoadingLedgerDetal2}
          title={CAP_SUBMIT}
      ...
      <TitleBarUpdated title={<span><span>
          <b style={{color:"#fff"}}>Ledger Detail 2 </b>
        </span>  
      ...
      {/* <div>
        <Table
          ...
          responseData={
            getAllLedgerListData2?.length ? getAllLedgerListData2 : []
          }
...
{/* updated code o?f new table */}
          ...
          <table className={style.newtable}>
            ...
            <tbody>
              {getLoadingLedgerDetal2 ? (
                ...
                ) : getAllLedgerListData2?.length ? (
                getAllLedgerListData2?.map((item, index) => {
                                  ...
                                  <td className={`${style.width9} ${style.clickAble}`} 
                                    ...
                                    onClick={(() => {
                                      if (item?.cashOrCredit ==('Cash'||'Credit')) {
                                        ...
                                        const newData = {userId:item?.userId, name:singleUserData?.userName, description:item?.description, marketId:item?.marketId, depositId: item?.originalId, roundId: item?.roundId, sportsId: item?.sportsId}
        ...
        {getAllLedgerListData2?.length ? (
          <PaginationStyled
            ...
            total={getLedgerDetailTotalRecords2 || 10}
